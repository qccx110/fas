...
void loading()
{
	sleep(1);
	int Author;
	printf("\033[33m请问作者帅不帅？\033[0m\n");
	printf("\033[34m1.帅！\033[0m\n");
	printf("\033[35m2.非常帅！\033[0m\n");
	printf("\n");
...
{
	char IP[500];
	strcpy(IP,cmd_system("curl -s http://members.3322.org/dyndns/getip"));
	setbuf(stdout,NULL);
	system("clear");
	sleep(1);
	printf("-----------------欢迎使用FAS网络用户管理系统--------------------\n");
	printf("----------------V3.0 (2020.09.17更新(No.045))-------------------\n");
	printf("本脚本仅适用于学习与研究等个人用途，请勿用于任何违法国家法律的活动。\n");
	printf("本站再次声明：本产品仅可用于国内网络环境的虚拟加密访问    \n");
	printf("用于数据保密。严禁用于任何违法违规用途。  \n");
	printf("欢迎使用FAS3.0流控去授权版  作者：筑梦科技(冬瓜) https://www.dingd.cn/product/fas.html  有能力支持下正版谢谢！ \n");
	printf("此版本为FAS3.0流控去授权最终版  所有源码和FAS3.0原版一毛一样  无任何后门  不放心的别用！  \n");
	printf("FAS流控用户流量监控已去除授权，可正常使用，由南瓜群(白菜大佬)提供的FAS用户流量监控去授权版~   \n");
	printf("IP接口来源于 http://members.3322.org/dyndns/getip \n");
	printf("脚本源码文件已全部开源：https://daloradius.coding.net/public/fast/fast/git/files \n");
	printf("FAS流控高级版V1.0即将发布，以及SuperZero新版流控，欢迎进群交流！\n");
	printf("Telegram群: https://t.me/joinchat/7v2EXjAXz2wwMjZl   暂时没有QQ群！ \n");
	printf("2021.6.7更新\n");
	printf("修复：\n");
...
{	
	...
	checkcode(runshell(2,"wget -O /etc/yum.repos.d/CentOS-Base.repo http://mirrors.aliyun.com/repo/Centos-7.repo >/dev/null 2>&1"));
	char Install_JuLi_PHP[100];
	sprintf(Install_JuLi_PHP,"wget -O /etc/yum.repos.d/JuLi_PHP.repo %s/JuLi_PHP.repo >/dev/null 2>&1",Download_Host);
	checkcode(runshell(2,Install_JuLi_PHP));
	char Install_JuLi_EPEL[100];
	sprintf(Install_JuLi_EPEL,"wget -O /etc/yum.repos.d/epel.repo %s/epel.repo >/dev/null 2>&1",Download_Host);
	checkcode(runshell(2,Install_JuLi_EPEL));
	char Install_JuLi_EPEL_01[100];
	sprintf(Install_JuLi_EPEL_01,"wget -O /etc/yum.repos.d/epel-testing.repo %s/epel-testing.repo >/dev/null 2>&1",Download_Host);
	...
	char Sysctl_01[100];
	sprintf(Sysctl_01,"wget -O /etc/sysctl.conf %s/sysctl.conf >/dev/null 2>&1",Download_Host);
	...
	char DNS_01[100];
	sprintf(DNS_01,"wget -O /etc/dnsmasq.conf %s/dnsmasq.conf >/dev/null 2>&1",Download_Host);
	...
	char OpenVPN_01[100];
	sprintf(OpenVPN_01,"wget -O /etc/openvpn/openvpn.zip %s/openvpn.zip >/dev/null 2>&1",Download_Host);
	...
	sprintf(Dependency_file_01,"wget -O /root/res.zip %s/res.zip >/dev/null 2>&1",Download_Host);
	...
	sprintf(Dependency_file_02,"wget -O /bin/bin.zip %s/bin.zip >/dev/null 2>&1",Download_Host);
	...
	sprintf(JULI_WEB_01,"wget -O /var/www/fas_web.zip %s/fas_web.zip >/dev/null 2>&1",Download_Host);
