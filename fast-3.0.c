...
void Index()
{
	//获取IP (IP接口来源于小欢网络科技 https://www.xh-ws.com/ )
	char IP[500];
	strcpy(IP,cmd_system("curl -s http://118.25.14.23/getip.php"));
	setbuf(stdout,NULL);
	system("clear");
	sleep(1);
	...
	printf("FAS流控用户流量监控已去除授权，可正常使用，由南瓜群 - 白菜 大佬提供的FAS用户流量监控去授权版~   \n");
	printf("IP接口来源于小欢网络科技 https://www.xh-ws.com/ \n");
